    ...
    with open(original_file, "r", encoding="utf-8") as f:
        ...
        for line in f:
            ...
            if match:
                ...
                print(f"⚠️ Skipped invalid line: {line.strip()}", file=sys.stderr)
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python replace.py <original_file> <fixes_json_file> <output_file>")
        sys.exit(1)
    
    original_file = sys.argv[1]
    fixes_json_file = sys.argv[2]
    output_file = sys.argv[3]
    
    with open(fixes_json_file, "r") as f:
        fixes = json.load(f)
    
    merge_fixed_snippets_into_file(original_file, fixes, output_file)